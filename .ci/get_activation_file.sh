...
#Set the name of the unity activation file. If the variable has not been set default to ./unity3d.alf
activation_file=${UNITY_ACTIVATION_FILE:-./unity3d.alf}

# verify that a username and password has been set and give an error message otherwise.
...
#Unity requires a display to exist but we do not have a display. We can work around this using the x virtual frame buffer -
# This xvfb-run command will  create a virtual display with resulution 640x480 and 24 bit color depth (unity requires 24 or 32)
# and this allows stuff to work. 
...
echo content of unity-output.log
